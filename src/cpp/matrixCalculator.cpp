...
public:
  ...
  int ActualRow() { return Actual(Row); }

  int ActualColumn() { return Actual(Column); }
...
class MatrixCalculator {
  ...
  static bool TestResult(vector<vector<int>> matrix, int expectedSum,
                         int side) {
    for (int rowIndex = 0; rowIndex < side; rowIndex++) {
      int sum = 0;
...
public:
  static MatrixResult Calculate(int side) {
    if (side % 2 != 1) {
      throw invalid_argument("Side needs to be an odd number.");
    }
    ...
    return MatrixResult(matrix, expectedSum, side,
